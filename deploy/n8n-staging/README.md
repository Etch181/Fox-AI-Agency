...
This is a separate, persistent, fail-closed n8n Compose project. The editor router is protected by Traefik BasicAuth and n8n owner login. FOX staging is currently connected through the signed `POST /webhook/fox-unified-events` router; outbound campaign actions remain disabled until an operator reviews credentials and policy constraints.
...
Set FOX staging only: `ENABLE_N8N=true`, `N8N_WEBHOOK_URL=http://n8n:5678/webhook/fox-unified-events`, and `N8N_WEBHOOK_SECRET` equal to the n8n-side shared secret. Never place these values in git or workflow JSON.
