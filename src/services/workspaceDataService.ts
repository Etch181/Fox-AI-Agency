...
export const workspaceDataService = {
  ...
  async createAppointment(
    ...
    try {
      ...
      const ws: any = wsSnap.exists ? wsSnap.data() : null;
      if (ws && isWorkspaceEntitlementActive(ws) && canWorkspaceUseFeature(ws, "appointment_reminder")) {
        const channel = (data.channel || "telegram") as any;
        const recipient = String(data.sessionId || "").startsWith(`${channel}:`)
          ? String(data.sessionId).split(":").slice(2).join(":")
          : String(data.sessionId || data.phone || "").trim();
        const appointmentDateTime = new Date(`${data.date}T${String(data.time).replace(/\s*(AM|PM)\s*$/i, " $1")}:00`);
        const fallbackTime = appointmentDateTime.getTime() > Date.now() ? appointmentDateTime : null;
        if (recipient && fallbackTime) {
          for (const hoursBefore of [24, 2]) {
            const scheduledAt = new Date(fallbackTime.getTime() - hoursBefore * 60 * 60 * 1000);
            if (scheduledAt.getTime() > Date.now()) {
              await scheduleReminder(workspaceId, {
                appointmentId: id,
                customerId: recipient,
                channel,
                scheduledAt: scheduledAt.toISOString(),
                timezone: "Africa/Cairo",
                messageContent: `تذكير 📅 لديك موعد يوم ${data.date} الساعة ${data.time}. نراك قريباً.`,
                dedupKey: `${workspaceId}_${id}_${hoursBefore}h`,
              });
            }
          }
        }
      ...
      console.warn("[FOX Appointment Reminder] Scheduling skipped safely:", reminderError?.message || reminderError);
    }
