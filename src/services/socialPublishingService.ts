...
export async function processScheduledPost(
  ...
  if (record.mode === 'MANUAL_APPROVAL') return { success: false, error: 'AWAITING_MANUAL_APPROVAL' };
...
export async function runDueMarketingPublishing(now: Date = new Date()): Promise<{ scanned: number; published: number; failed: number; skipped: number }> {
  const snapshot = await adminDb.collection('socialPublishingRecords')
    .where('state', '==', 'scheduled').get();
  let published = 0, failed = 0, skipped = 0;
  for (const doc of snapshot.docs) {
    const record = doc.data() as SocialPublishRecord;
    if (!record.scheduledAt || record.scheduledAt > now.toISOString()) continue;
    // Scheduled MANUAL_APPROVAL records remain queued until an owner explicitly approves.
    if (record.mode !== 'AUTO_PUBLISH') { skipped++; continue; }
    const result = await processScheduledPost(String(record.workspaceId), doc.id);
    if (result.success) published++; else failed++;
  }
  return { scanned: snapshot.size, published, failed, skipped };
}
