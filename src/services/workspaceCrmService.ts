...
function detectCustomerIntent(message: string): {
  ...
  const cancellationWords = [
    "إلغاء الحجز", "الغاء الحجز", "ألغي الحجز", "الغي الحجز",
    "إلغاء الموعد", "الغاء الموعد", "ألغي الموعد", "الغي الموعد",
    "cancel booking", "cancel appointment", "cancel my booking",
    "cancel my appointment", "الغاء", "إلغاء", "اتلغى الحجز", "اتلغى الموعد"
  ];

  if (cancellationWords.some((word) => text.includes(word.toLowerCase()))) {
    return {
  ...
  if (bookingWords.some((word) => text.includes(word))) {
    return {
      intent: "booking",
