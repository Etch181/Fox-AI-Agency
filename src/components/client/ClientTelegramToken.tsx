  ...
  const handleVerifyToken = async () => {
    ...
    try {
      ...
      if (data.success && data.botInfo) {
        setBotDetails({
          id: data.botInfo.id,
          first_name: data.botInfo.first_name || data.botInfo.username || "",
          username: data.botInfo.username || botName.replace(/^@/, ""),
          can_join_groups: data.botInfo.can_join_groups !== false,
        });
      } else {
        const errorMsg = data.error || data.description || (isAr ? "رمز غير صالح. تحقق من BotFather." : "Invalid token. Check BotFather.");
        alert(isAr ? errorMsg : errorMsg);
      }
    } catch (err) {
