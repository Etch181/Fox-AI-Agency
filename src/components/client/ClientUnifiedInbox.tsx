...
export const ClientUnifiedInbox: React.FC = () => {
  ...
  const handleTakeover = async () => {
    if (!activeConv || takingOver || activeConv.assignedTo === "human") return;

    const approved = confirm(
      isAr
        ? "إيقاف رد البوت على هذه المحادثة وتحويلها لموظف بشري الآن؟"
        : "Pause the bot and take over this conversation as a human agent?"
    );
    if (!approved) return;
    ...
    try {
      const res = await authenticatedFetch(
        `/api/conversations/${encodeURIComponent(activeConv.id)}/takeover`,
        {
          method: "POST",
          body: JSON.stringify({
            workspaceId: currentWorkspace.id,
            reason: "manual_owner_takeover",
          }),
        }
      );
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || "Failed to activate human takeover");
    } catch (error: any) {
      alert(error?.message || (isAr ? "تعذر استلام المحادثة." : "Could not take over conversation."));
    } finally {
      setTakingOver(false);
    }
  };
              ...
              <div className="flex items-center gap-2">
                {activeConv.assignedTo === "human" ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-black text-white hover:bg-indigo-500 disabled:opacity-50"
                  >
                    <Bot className="h-3.5 w-3.5" />
                    {returningToAI ? (isAr ? "جاري التفعيل..." : "Activating...") : (isAr ? "تفعيل البوت" : "Resume AI")}
                  </button>
                ) : (
                  <button
                    type="button"
                    ...
                    disabled={takingOver}
                    className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-2 text-[10px] font-black text-white hover:bg-amber-600 disabled:opacity-50"
                  >
                    <UserCheck className="h-3.5 w-3.5" />
                    {takingOver ? (isAr ? "جاري الاستلام..." : "Taking over...") : (isAr ? "استلام بشري" : "Take over")}
                  </button>
                )}
            ...
            >
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={sendingHumanReply}
                  placeholder={
                    activeConv.assignedTo === "human"
                      ? isAr
                        ? `اكتب ردك للعميل عبر ${activeConv.channel}...`
                        : `Reply to customer via ${activeConv.channel}...`
                      : isAr
                      ? "اضغط استلام بشري لإيقاف البوت والرد بنفسك..."
                      : "Take over first to pause the bot and reply manually..."
                  }
