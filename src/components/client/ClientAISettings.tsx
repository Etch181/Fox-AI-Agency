...
export const ClientAISettings: React.FC = () => {
  ...
  const saveGeminiKey = async (remove = false) => {
    setSavingGeminiKey(true); setGeminiMessage("");
    try {
      const response = await authenticatedFetch(`/api/workspaces/${currentWorkspace.id}/ai/gemini-key`, {
        method: remove ? "DELETE" : "PUT",
        headers: { "Content-Type": "application/json" },
        body: remove ? undefined : JSON.stringify({ apiKey: geminiApiKey }),
      });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.error || "Gemini key update failed");
      setGeminiConfigured(Boolean(payload.configured)); setGeminiApiKey("");
      setGeminiMessage(remove ? (isAr ? "تم حذف مفتاح Gemini." : "Gemini key removed.") : (isAr ? "تم حفظ مفتاح Gemini بشكل مشفر. سيُستخدم تلقائياً كمسار احتياطي." : "Gemini key stored encrypted and will be used automatically as fallback."));
    } catch (error) { setGeminiMessage(error instanceof Error ? error.message : "Gemini key update failed"); }
  ...
  return (
    <div className={`space-y-6 animate-fade-in ${isAr ? "dir-rtl" : "dir-ltr"}`}>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-3">
          <div><h2 className="font-extrabold text-slate-900 dark:text-white">{isAr ? "مفتاح Gemini الاحتياطي" : "Gemini Fallback Key"}</h2>
          <p className="text-xs text-slate-500">{geminiConfigured ? (isAr ? "مفتاح منشأتك محفوظ بشكل مشفر." : "Your workspace key is stored encrypted.") : (isAr ? "اختياري — OpenRouter يظل المزود الأساسي." : "Optional — OpenRouter remains primary.")}</p></div>
          <Cpu className="h-5 w-5 text-indigo-500" />
        </div>
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <input type="password" value={geminiApiKey} onChange={e => setGeminiApiKey(e.target.value)} placeholder="AIza..." autoComplete="off" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800" />
          <button type="button" disabled={savingGeminiKey || !geminiApiKey.trim()} onClick={() => void saveGeminiKey(false)} className="rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{isAr ? "حفظ مشفر" : "Save Encrypted"}</button>
          <button type="button" disabled={savingGeminiKey || !geminiConfigured} onClick={() => void saveGeminiKey(true)} className="rounded-xl border border-rose-300 px-4 py-2 text-xs font-bold text-rose-600 disabled:opacity-50">{isAr ? "حذف" : "Remove"}</button>
        </div>
        <p className="mt-2 text-[11px] text-slate-500">{isAr ? "عند انتهاء quota أو rate limit، FOX ينتقل تلقائياً للمفتاح الاحتياطي التالي إذا تم إعداد GEMINI_API_KEYS على الخادم." : "On quota/rate-limit errors FOX automatically rotates to the next configured fallback key."}</p>
        {geminiMessage && <p className="mt-2 text-xs text-slate-600 dark:text-slate-300">{geminiMessage}</p>}
      </section>
