...
export const PublicLandingPage: React.FC<PublicLandingPageProps> = ({ onLoginClick }) => {
  const { plans, registerWorkspace, loginWithEmail, language, setLanguage, addToast } = useApp();
  ...
  const sendEmailOtp = async () => {
    ...
    if (!email || !email.includes("@")) {
      addToast(isAr ? "أدخل بريد إلكتروني صحيح أولاً." : "Enter a valid email first.", "error");
      return;
    }
    setOtpBusy(true);
    try {
      const response = await fetch("/api/send-otp", {
        ...
        body: JSON.stringify({ email, ownerName, workspaceName: wsName }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || !payload.success) throw new Error(payload.error || "OTP_SEND_FAILED");
      setOtpSent(true);
      setOtpVerified(false);
      addToast(isAr ? "تم إرسال كود التفعيل إلى بريدك." : "Verification code sent to your email.", "success");
    } catch (error: any) {
      addToast(error?.message || (isAr ? "تعذر إرسال الكود." : "Could not send the code."), "error");
    } finally {
      setOtpBusy(false);
    }
  };
  ...
  const verifyEmailOtp = async () => {
    ...
    try {
      const response = await fetch("/api/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: regEmail.trim().toLowerCase(), otpCode: emailOtp.trim() }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok || !payload.success || !payload.verified) throw new Error(payload.error || "OTP_INVALID");
      setOtpVerified(true);
      setOtpVerificationToken(String(payload.verificationToken || ""));
      addToast(isAr ? "تم تأكيد البريد الإلكتروني بنجاح." : "Email verified successfully.", "success");
    } catch (error: any) {
      setOtpVerified(false);
      setOtpVerificationToken("");
      addToast(error?.message || (isAr ? "كود التفعيل غير صحيح." : "Invalid verification code."), "error");
    } finally {
      setOtpBusy(false);
    }
  };
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!otpVerified || !otpVerificationToken) {
      addToast(
        isAr ? "يجب تأكيد البريد الإلكتروني بكود OTP قبل إنشاء الحساب." : "Verify your email with the OTP before creating the account.",
        "error"
      );
      return;
    }
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pt-4 sm:pt-8">
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4">
              ...
              >
                <span>{isAr ? "إنشاء حساب باستخدام كود التفعيل" : "Create Account with Activation Code"}</span>
                ...
                <form onSubmit={handleRegisterSubmit} className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {isAr
                        ? "أنشئ مساحة عملك باستخدام كود التفعيل المرسل لك من FOX"
                        : "Use your FOX activation code to activate your business workspace"}
                    </p>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/80 px-3.5 py-2.5 focus-within:border-orange-500">
                        ...
                        <input
                          ...
                          value={regEmail}
                          onChange={(e) => { setRegEmail(e.target.value); setOtpVerified(false); setOtpVerificationToken(""); setOtpSent(false); setEmailOtp(""); }}
                          className="w-full text-xs font-semibold bg-transparent text-white focus:outline-none placeholder-slate-500"
                          required
                        />
                      </div>
                  ...
                  <div className="rounded-2xl border border-slate-700 bg-slate-900/70 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-black text-white">{isAr ? "تأكيد البريد الإلكتروني" : "Verify Email"}</p>
                        <p className="mt-1 text-[10px] text-slate-400">{isAr ? "سيتم إرسال كود صالح لمدة 15 دقيقة." : "A code valid for 15 minutes will be sent."}</p>
                      </div>
                      <button type="button" onClick={sendEmailOtp} disabled={otpBusy || !regEmail} className="rounded-xl bg-slate-700 px-3 py-2 text-[10px] font-black text-white hover:bg-slate-600 disabled:opacity-50">
                        {otpBusy ? "..." : otpSent ? (isAr ? "إعادة إرسال" : "Resend") : (isAr ? "إرسال الكود" : "Send OTP")}
                      </button>
                    </div>
                    {otpSent && (
                      <div className="mt-3 flex gap-2">
                        <input value={emailOtp} onChange={(e) => setEmailOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" maxLength={6} placeholder="123456" className="flex-1 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-center text-sm font-black tracking-[0.3em] text-white" />
                        <button type="button" onClick={verifyEmailOtp} disabled={otpBusy || emailOtp.length !== 6} className="rounded-xl bg-emerald-600 px-4 py-2 text-[10px] font-black text-white disabled:opacity-50">{isAr ? "تأكيد" : "Verify"}</button>
                      </div>
                    )}
                    {otpVerified && <p className="mt-2 text-[10px] font-black text-emerald-400">✓ {isAr ? "البريد مؤكد" : "Email verified"}</p>}
                  </div>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-300 block mb-1">
                      {isAr ? "كود التفعيل الصادر من FOX" : "FOX Activation Code"}
                    </label>
                    <div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/80 px-3.5 py-2.5 focus-within:border-orange-500">
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-800/60">
                        ...
                        <tr>
                          ...
                          <td className="p-3"><span className="px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-400 font-bold">ACTIVATION</span></td>
