...
// FOX SAFE STORAGE PARSER: corrupted/legacy browser storage must never crash the SPA on refresh.
function safeStorageJson<T>(raw: string | null, fallback: T): T {
  if (!raw) return fallback;
  try {
    return JSON.parse(raw) as T;
  } catch (error) {
    console.warn("[FOX STORAGE] Ignoring invalid localStorage value:", error);
    return fallback;
  }
}
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Safety watchdog: Firebase auth/profile restoration must never leave the SPA
  // on an infinite loading screen. If the auth SDK or profile read stalls,
  // fail closed to the login portal instead of rendering a blank/loading page.
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      if (!authHydrated && mountedRef.current) {
        console.warn("[FOX AUTH] Hydration watchdog fired; returning to login portal.");
        setCurrentUser(null);
        setWorkspaces([]);
        setWorkspacesLoading(false);
        setAuthHydrated(true);
        authHydrationRef.current = { uid: "", status: "failure", user: null };
        try {
          if (auth.currentUser) await firebaseSignOut(auth);
        } catch (error) {
          console.warn("[FOX AUTH] Watchdog sign-out failed:", error);
        }
      }
    }, 15000);
  ...
  const [plans, setPlans] = useState<SubscriptionPlan[]>(() => {
    ...
    return safeStorageJson(saved, INITIAL_PLANS);
  });
  ...
  const qdrantSyncSignatureRef = useRef("");

  // Backfill approved tenant knowledge into Qdrant once per content signature.
  // This covers facts created before Qdrant indexing was enabled.
  useEffect(() => {
    if (!currentUser || !currentWorkspaceId || knowledgeFacts.length === 0) return;
    const approvedFacts = knowledgeFacts
      .filter((fact) => fact.workspaceId === currentWorkspaceId && fact.approved === true)
      .map((fact) => ({ id: fact.id, question: fact.question, answer: fact.answer, approved: true }))
      .sort((a, b) => a.id.localeCompare(b.id));
    if (approvedFacts.length === 0) return;
    const signature = JSON.stringify(approvedFacts);
    const signatureKey = `${currentWorkspaceId}:${signature}`;
    if (qdrantSyncSignatureRef.current === signatureKey) return;
    qdrantSyncSignatureRef.current = signatureKey;

    void authenticatedFetch("/api/knowledge/qdrant/index", {
      method: "POST",
      body: JSON.stringify({ workspaceId: currentWorkspaceId, facts: approvedFacts }),
    }).then((response) => {
      if (!response.ok) throw new Error(`QDRANT_SYNC_HTTP_${response.status}`);
    }).catch((err) => {
      qdrantSyncSignatureRef.current = "";
      console.warn("Qdrant approved knowledge backfill deferred:", err);
    });
  }, [currentUser?.id, currentWorkspaceId, knowledgeFacts]);
  ...
  useCollectionSync("doctors", setDoctors);
  useCollectionSync("clinicServices", setClinicServices);
  useCollectionSync("courses", setCourses);
  useCollectionSync("courseReviews", setCourseReviews);
  ...
  const registerWorkspace = async (
    ...
    // Public self-registration no longer grants a trial. A FOX-issued activation code is required.
    if (!initialCode?.trim()) {
      addToast(language === "ar" ? "التسجيل يحتاج كود تفعيل صادر من FOX." : "Registration requires a FOX activation code.", "error");
      return null;
    }
    ...
    try {
      ...
      const provisioningResponse = await authenticatedFetch(
        "/api/registration/provision-workspace",
        {
          ...
          body: JSON.stringify({
            ...
            phone: cleanPhone,
            industry,
            activationCode: initialCode.trim(),
      ...
      if (
        ...
        !["business", "enterprise"].includes(authoritativeWorkspace.planId) ||
        !Number.isFinite(
  ...
  const addCourse = (course: Omit<CourseItem, "id" | "workspaceId">) => {
    ...
    const newCourse: CourseItem = { ...course, id: `course_${Math.random().toString(36).substring(2, 9)}`, workspaceId: currentWorkspace.id };
    setCourses((prev) => [newCourse, ...prev]);
    setDoc(doc(db, "courses", newCourse.id), newCourse).catch((error) => console.warn("[FOX Courses] Create sync failed:", error));
    addToast("Course saved to CRM", "success");
  };
  const updateCourse = (id: string, updates: Partial<CourseItem>) => {
    const existing = courses.find((c) => c.id === id);
    if (!existing || (!isSuperAdmin && existing.workspaceId !== currentWorkspace?.id)) return;
    setCourses((prev) => prev.map((c) => (c.id === id ? { ...c, ...updates } : c)));
    updateDoc(doc(db, "courses", id), updates).catch((error) => console.warn("[FOX Courses] Update sync failed:", error));
  };
  const deleteCourse = (id: string) => {
    const existing = courses.find((c) => c.id === id);
    if (!existing || (!isSuperAdmin && existing.workspaceId !== currentWorkspace?.id)) return;
    setCourses((prev) => prev.filter((c) => c.id !== id));
    deleteDoc(doc(db, "courses", id)).catch((error) => console.warn("[FOX Courses] Delete sync failed:", error));
  };
  ...
  const addCourseReview = (review: Omit<CourseReview, "id" | "workspaceId" | "date">) => {
    ...
    setDoc(doc(db, "courseReviews", newReview.id), newReview).catch((error) => console.warn("[FOX Course Reviews] Create sync failed:", error));
  };

  const updateCourseReview = (id: string, updates: Partial<CourseReview>) => {
    const existing = courseReviews.find((r) => r.id === id);
    if (!existing || (!isSuperAdmin && existing.workspaceId !== currentWorkspace?.id)) return;
    setCourseReviews((prev) => prev.map((r) => (r.id === id ? { ...r, ...updates } : r)));
    updateDoc(doc(db, "courseReviews", id), updates).catch((error) => console.warn("[FOX Course Reviews] Update sync failed:", error));
  };
  ...
  const deleteCourseReview = (id: string) => {
    ...
    deleteDoc(doc(db, "courseReviews", id)).catch((error) => console.warn("[FOX Course Reviews] Delete sync failed:", error));
  };
  ...
  const approveKnowledgeFact = (factId: string) => {
    ...
    const approvedFact = { ...existing, approved: true };
    setKnowledgeFacts((prev) => prev.map((k) => (k.id === factId ? approvedFact : k)));
    updateDoc(doc(db, "knowledgeFacts", factId), { approved: true }).catch(console.error);
    void authenticatedFetch("/api/knowledge/qdrant/index", {
      method: "POST",
      body: JSON.stringify({ workspaceId: existing.workspaceId, facts: [approvedFact] }),
    }).catch((err) => console.warn("Qdrant approval indexing deferred:", err));
  ...
  const rejectKnowledgeFact = (factId: string) => {
    ...
    setKnowledgeFacts((prev) => prev.filter((k) => k.id !== factId));
    deleteDoc(doc(db, "knowledgeFacts", factId)).catch(console.error);
    void authenticatedFetch("/api/knowledge/qdrant/delete", {
      method: "POST",
      body: JSON.stringify({ workspaceId: existing.workspaceId, factId }),
    }).catch((err) => console.warn("Qdrant knowledge deletion deferred:", err));
  ...
  const addKnowledgeFact = async (fact: Omit<KnowledgeBaseFact, "id" | "createdAt">) => {
    ...
    try { await authenticatedFetch("/api/knowledge/qdrant/index", { method: "POST", body: JSON.stringify({ workspaceId: targetWsId, facts: [newFact] }) }); } catch (err) { console.warn("Qdrant knowledge indexing deferred:", err); }
